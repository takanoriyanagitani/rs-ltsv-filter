...
fn stdin2stdout(line_sep: u8, field_sep: u8, value_sep: u8, label: &[u8]) -> Result<(), io::Error> {
    let i = io::stdin();
    let il = i.lock();
    let o = io::stdout();
    let ol = o.lock();
    rs_ltsv_filter::single::line2single2value2output::reader2writer(
        il, ol, line_sep, field_sep, value_sep, label,
    )
}
...
fn sub() -> Result<(), io::Error> {
    ...
    stdin2stdout(line_sep, field_sep, value_sep, lb)
}
